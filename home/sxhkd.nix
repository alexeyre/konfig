{ pkgs, config, ... }:
{
  services.sxhkd = {
	...
	"super + semicolon ; p" = "${pass} -c \$(${pass} ls --flat | dmenu)";
        ...
        "super + alt + ctrl + p" = "for x in \$(${bspc} query -D -d .\\!occupied --names | ${sed} \'\/browser\/d\;\/terminal\/d\;\/emacs\/d\;\/music\/d\;\/msg\/d\;'); do ${bspc} desktop \"\$x\" -r; done";
	...
	"super + q ; t" = "${bspc} desktop -f terminal";
        ...
        "super + a ; e" = "emacsclient -c -a \"emacs\" -e \"(alex/goto-dashboard)\"";
      };
	};
}
