{ config, lib, pkgs, ... }:
with lib;
let arm64jdk = lowPrio (pkgs.openjdk15_headless.overrideAttrs (old: rec {
...
arm64gradle = pkgs.gradle.overrideAttrs ( old: rec {
  ...
  src = pkgs.fetchurl {
    url = "https://services.gradle.org/distributions-snapshots/${name}-bin.zip";
...
in
  {
    ...
    options.alex.java.bloat = mkOption {
      type = types.bool;
      default = false;
      description = "Whether to configure a bloated, disgusting environment for disgusting, inf1b java development 🤮";
    };
    config = mkIf config.alex.java.enable {
      home.packages = lib.mkIf config.alex.java.enable (if config.alex.is-mac then [ arm64jdk arm64gradle ] else [ jdk ]);
