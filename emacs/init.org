...
xwidget things
#+begin_src emacs-lisp
   (add-hook 'xwidget-webkit-mode-hook '(lambda () "" (setq-local default-input-method "programmer-dvorak")))
#+end_src

#+RESULTS:
| lambda | nil |   | (setq-local default-input-method programmer-dvorak) |
| lambda | nil |   | (set-input-method programmer-dvorak)                |
...
#+RESULTS:
| doom-modeline-mode | #[0 \300 \207 [dashboard-insert-startupify-lists] 1] | tramp-register-archive-file-name-handler |
